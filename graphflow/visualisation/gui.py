from tkinter import Tk, ttk, filedialog, Button, messagebox
...
class Gui:
    ...
    def _load_file(self, root, **kwargs):
        # frame.network = filedialog.askopenfilename(initialdir="/", title="Select network file",
        #                                          filetypes=(("json files", "*.json"), ("all files", "*.*")))
        root.filename = filedialog.askopenfilename(title="Select network file", **kwargs)

    def _generate_metrics_checklist(self, frame):
        # metrics = ("Author", "John", "Mohan", "James", "Ankur", "Robert")
        metrics = dir(mtr)
        regex = re.compile('__*')
        metrics = [x for x in metrics if not regex.match(x)]
        metrics.remove('calculate_metric')
        metrics.remove('calculate_metric_array')
    ...
    def _calculate_simple(self, metrics: [str] = None):
        ...
        with open(self.root.filename) as file:
            ...
            if metrics:
                self.root.calculated_metrics = mtr.calculate_metric_array('simple', self.root.solved_network, metrics)
    ...
    def _calculate_extended(self, metrics: [str] = None):
        ...
        with open(self.root.filename) as file:
            ...
            if metrics:
                self.root.calculated_metrics = mtr.calculate_metric_array('simple', solved_network, metrics)
    ...
    def _calculate_epidemic(self, metrics: [str] = None):
        raise NotImplementedError
